...
class genome
{
 ...
 //+++++++++ FUNCTIONS+++++++//
    ...
    double gasdev(long*);
	double ran2 (long*);
    void base (int, int, int);  //this function will set the first valuse and also builds the adjacency matrix and every gene in memory.
    void connect (int, int, double);
    void du_connect (int, int, double);
    bool checker (string);
    //void Prf_attch ();
	//void Pref_attach_dup (int);
    void Reader(string);
    void Initializing();
    void specefication();
    void duplication ();
    ...
    //Deleted__void second_initializer();
    void Remover();
