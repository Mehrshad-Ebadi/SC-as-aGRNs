...
void genome::base (int N)
{
    ...
    omega = 0;          //the exact amount of fitness fucntion
    du_omega = 0;
    
    ref_omega = 0;      //reference conditions, single
    du_ref_omega = 0;   //reference conditions, for duplicated
    ...
    craziness_counter = 10000; //counting number of cicle in a loop, to check if there is any infinite loops or not.
    ...
    ofstream OMEGA ("./Outputs/Omega.txt");
    ofstream du_OMEGA ("./Outputs/du_Omega.txt");

    int extinct = 0;
    int du_extinct = 0;
    int du_number_networks = number_networks;
    ...
    */
    ...
    { 
        ...
        { 
            initial_evaluation(location);
        }
        if (du_checker(du_location) == true)
        {
            du_initial_evaluation(du_location);
        }
    }
    
    Alive << ref_Envmnt << '\t' << ref_alive<<endl;
    du_Alive << ref_Envmnt << '\t' << du_ref_alive <<endl;
    OMEGA << ref_Envmnt << '\t' << ref_omega / ref_omega <<endl;
    du_OMEGA << ref_Envmnt << '\t' << du_ref_omega / du_ref_omega <<endl;
    ...
    while (single == true || dubl == true) // the simulation continues running until population of single, duplicates or both reach a small amount (less than 10 networks)
    {
        ...
        OMEGA << evolve << '\t' << double (omega / ref_omega) <<endl;
        du_OMEGA << evolve << '\t' << double (du_omega / du_ref_omega) <<endl;
